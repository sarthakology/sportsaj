...
const pageHeading = 'Buy Now';
const pageSubheading = 'Tickets coming soon. Register your interest and we\'ll notify you when sales open.';
    ...
    return (
      <div className="text-center py-10 sm:py-14">
        ...
        <p className="text-brand-gray text-sm max-w-sm mx-auto">
          Thanks for your interest in El Clásico Legends — {event.city}. We&apos;ll reach out when tickets go on sale.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
        <div>
          <label htmlFor={`${event.id}-name`} className="block text-brand-black text-sm font-medium mb-1.5">
            Full Name
          </label>
          <input
            id={`${event.id}-name`}
        ...
        <div>
          <label htmlFor={`${event.id}-email`} className="block text-brand-black text-sm font-medium mb-1.5">
            Email
          </label>
          <input
            id={`${event.id}-email`}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
        <div>
          <label htmlFor={`${event.id}-phone`} className="block text-brand-black text-sm font-medium mb-1.5">
            Phone
          </label>
          <input
            id={`${event.id}-phone`}
        ...
        <div>
          <label htmlFor={`${event.id}-tickets`} className="block text-brand-black text-sm font-medium mb-1.5">
      ...
      <div>
        <label htmlFor={`${event.id}-message`} className="block text-brand-black text-sm font-medium mb-1.5">
...
export default function BuyNow() {
  const [activeTab, setActiveTab] = useState('ahm');
  ...
  return (
    <>
      <SEO
        title="Buy Now"
        description="Register your interest for El Clásico Legends tickets in Ahemdabad and Kolkata."
        breadcrumb="Buy Now"
      />
          ...
          <div className="card card-pad lg:p-10 border-t-4 border-t-brand-red">
            <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-6 mb-6 sm:mb-8 pb-6 sm:pb-8 border-b border-brand-gray-light">
              <img
                ...
                alt={`El Clásico Legends — ${activeEvent.city}`}
            ...
            <InterestForm key={activeEvent.id} event={activeEvent} />
          </div>
        </div>
      </section>
    </>
